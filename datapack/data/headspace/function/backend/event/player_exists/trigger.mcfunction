...
# Enable headspace for all players unless defaults were changed by admin
execute if score [Headspace] hs.setts.EnablePlayersByDefault matches 1 run tag @s[predicate=!headspace:headspace_user] add hs.tag.HeadspaceUser
...
execute if score @s hs.check.ConditionsPassed matches 1 if score [Headspace] hs.setts.EnablePackAnnouncements matches 1 if predicate headspace:headspace_user run function headspace:backend/alert/datapack_updated
