$execute in $(dimension) unless entity @s[tag=hs.tag.ForceloadedHeadspaceChunk] store success score @s hs.check.PermanentChunk run forceload query $(x) $(z)
...
$execute at @s as @a[distance=..1.5,sort=furthest,predicate=headspace:on_headspace_bed,predicate=!headspace:sneaking,predicate=headspace:headspace_user] in $(dimension) if block $(x) $(y_bed) $(z) minecraft:blue_bed run tp $(x) $(y) $(z)
