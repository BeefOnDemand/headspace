...
$execute if score @s hs.check.BedLocated matches 1 at @s as @a[distance=..2,sort=furthest,predicate=headspace:on_headspace_bed,predicate=!headspace:sneaking,predicate=headspace:headspace_user] in $(dimension) run tp $(x) $(y) $(z)
