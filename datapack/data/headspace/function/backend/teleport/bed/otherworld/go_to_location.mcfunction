...
$execute at @s as @a[distance=..1.5,sort=furthest,predicate=headspace:on_otherworld_bed,predicate=!headspace:sneaking,predicate=headspace:headspace_user] in $(dimension) if score @s hs.check.BedLocated matches 1 run tp $(x) $(y) $(z)
